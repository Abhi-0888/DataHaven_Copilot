...
import {
  ...
  Sparkles, Link2, Download, BarChart2, Bot, Info, ShieldAlert, Shield, Brain, ChevronRight
} from "lucide-react";
...
import { useDataset, useDatasetReport, useAnalyzeDataset, useTimeline, useLedger, useTrustBreakdown } from "@/hooks/use-datasets";
import { useCopilotQuery } from "@/hooks/use-copilot";
import { useRegisterBlockchain } from "@/hooks/use-blockchain";
...
type Tab = "overview" | "trust" | "timeline" | "ledger" | "analysis" | "copilot";
...
export default function DatasetDetails() {
  ...
  const { data: timeline, isLoading: isTimelineLoading } = useTimeline(datasetId);
  const { data: ledger, isLoading: isLedgerLoading } = useLedger(datasetId);
  const { data: trust, isLoading: isTrustLoading } = useTrustBreakdown(datasetId);
  ...
  const handleRegister = () => {
    registerMutation.mutate(datasetId, {
      onSuccess: (data: any) => {
        toast({
          title: "Registered on DataHaven Testnet",
          description: `Tx Hash: ${data.txHash.substring(0, 20)}...`,
        });
      },
      onError: (err) => {
  ...
  const isVerified = dataset.trustScore > 80;
  const mockChartData = [
  ...
  const tabs: { id: Tab; label: string; icon: any }[] = [
    ...
    { id: "ledger", label: "Ledger", icon: Brain },
  ...
  return (
    <AppLayout>
      <div className="mb-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 flex-wrap text-sm text-muted-foreground">
                <span className="flex items-center gap-1.5 bg-black/40 px-3 py-1 rounded-full border border-white/5 text-foreground">
                  {isVerified ? <ShieldCheck className="w-4 h-4 text-success" /> : <ShieldAlert className="w-4 h-4 text-yellow-500" />}
                  {dataset.trustScore.toFixed(1)} Trust
                </span>
                <ProofBadge verified={isVerified} />
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 border-b border-white/10 pb-px overflow-x-auto scrollbar-hide">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-3 font-medium text-sm capitalize transition-colors relative whitespace-nowrap
                  ${activeTab === tab.id ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}
              >
                <Icon className="w-4 h-4" />
                {tab.label}
                {activeTab === tab.id && (
                  <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary shadow-[0_0_10px_rgba(139,92,246,0.5)]" />
                )}
              </button>
            );
          })}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                <div className="glass-panel p-6 rounded-2xl">
                  <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Info className="w-5 h-5 text-primary" /> About Dataset</h3>
                  <p className="text-muted-foreground leading-relaxed">
                    {dataset.description || "No detailed description. Run an AI analysis to generate metadata."}
                  </p>
                </div>
                  ...
                  <div className="glass-panel p-6 rounded-2xl">
                    ...
                    <div className="space-y-4 font-mono text-sm">
                      ...
                      <div>
                        ...
                        <div className="truncate bg-black/40 px-2 py-1 rounded border border-white/5 text-xs">{dataset.fileHash}</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground/70 mb-1">Metadata Hash</div>
                        <div className="truncate bg-black/40 px-2 py-1 rounded border border-white/5 text-xs">{dataset.metadataHash}</div>
                      </div>
                    </div>
                  </div>
                  <div className="glass-panel p-6 rounded-2xl flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col justify-center gap-5">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono text-xs truncate w-32">{dataset.storageId}</span>
                      </div>
                      <div className="flex justify-between items-center">
              ...
              <div className="space-y-6">
                ...
                <div className="glass-panel p-6 rounded-2xl bg-gradient-to-br from-card to-primary/5">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">Chat with your data using our advanced LLM models.</p>
            ...
            <div className="max-w-2xl">
              {isTrustLoading ? (
                ...
                <div className="glass-panel p-10 text-center rounded-2xl">
                  <p className="text-muted-foreground">Trust data unavailable.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "timeline" && (
            <div className="glass-panel p-6 rounded-2xl">
              <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                <Activity className="w-5 h-5 text-primary" /> Verification Timeline
              </h3>
              {isTimelineLoading ? (
          ...
          {activeTab === "ledger" && (
            <div>
              <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                <Brain className="w-5 h-5 text-primary" /> AI Memory Ledger
              </h3>
              {isLedgerLoading ? (
                <div className="space-y-4">
                  {[1, 2].map((i) => <div key={i} className="glass-panel h-32 rounded-2xl animate-pulse bg-white/5" />)}
                </div>
              ) : (
          ...
          {activeTab === "analysis" && (
            <div className="space-y-6">
              {isReportLoading ? (
                ...
                <>
                  <div className="glass-panel p-6 rounded-2xl">
                    <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><Sparkles className="w-5 h-5 text-primary" /> AI Summary</h3>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="glass-panel p-6 rounded-2xl">
                      ...
                      <div className="space-y-4">
                        {report.insights?.length > 0 ? report.insights.map((insight: any) => (
                          ...
                          <div className="p-4 rounded-xl bg-black/40 text-muted-foreground text-center">No insights generated yet.</div>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="glass-panel rounded-2xl flex flex-col h-[600px] border border-primary/20 shadow-[0_0_30px_rgba(139,92,246,0.1)] overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gradient-to-b from-transparent to-black/20">
                {chatHistory.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto opacity-50">
                    ...
                    <p className="text-sm">Ask questions about trends, anomalies, or request specific aggregations.</p>
                  </div>
                ) : (
                  chatHistory.map((msg, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-4 max-w-[80%] ${msg.role === "user" ? "ml-auto flex-row-reverse" : ""}`}>
                      ...
                      <div className={`p-4 rounded-2xl ${msg.role === "user" ? "bg-primary/20 rounded-tr-sm" : "bg-white/10 rounded-tl-sm"}`}>{msg.content}</div>
                    </motion.div>
                  ))
                )}
                {copilotMutation.isPending && (
                  <div className="flex gap-4 max-w-[80%]">
                    <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center shrink-0"><Bot className="w-5 h-5 text-white" /></div>
              ...
              <div className="p-4 bg-black/40 border-t border-white/5">
                <form onSubmit={handleQuery} className="relative">
                  <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Ask Copilot about this dataset..."
                  ...
                  <button type="submit" disabled={!query.trim() || copilotMutation.isPending}
